...
echo ""
echo "  2. Complete the OpenAI Codex OAuth flow to get tokens:"
echo "     openclaw onboard --auth-choice openai-codex --skip-daemon"
echo ""
echo "  3. Copy the repo's example config and extract your tokens:"
echo "     cd ~/redbot-provision"
echo "     cp .env.example .env"
echo ""
echo "     # Print the access token (copy the output):"
echo "     jq -r '.profiles[\"openai-codex:default\"].access' ~/.openclaw/agents/main/agent/auth-profiles.json"
echo "     # Print the refresh token (copy the output):"
echo "     jq -r '.profiles[\"openai-codex:default\"].refresh' ~/.openclaw/agents/main/agent/auth-profiles.json"
echo ""
echo "  4. Paste tokens into .env and fill in the rest:"
echo "     nano .env    # Set OPENAI_ACCESS_TOKEN, OPENAI_REFRESH_TOKEN, bot identity, email, port"
echo "     ./setup.sh"
echo ""
